import streamlit as st  # a low-code web framework
import pandas as pd  # a data analysis and wrangling tool
...
with st.sidebar:
    ...
    year_listed = list(df_reshaped.year.unique())[::-1]

    selected_year = st.selectbox("select a year", year_listed, index=len(year_listed)-1)
...
def calculation_population_difference(input_df, input_year):
...
with col[0]:
    ...
    df_population_difference_sorted = calculation_population_difference(df_reshaped, selected_year)
